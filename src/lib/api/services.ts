...
import { SUBSCRIPTION_PAYMENT_ENABLED, WEEKDAYS } from "@/constants/config";
import {
 SUBSCRIPTION_PLANS,
 ...
 */
async function fetchTherapistPublic(
 therapistId?: string
): Promise<BackendTherapistPublic | null> {
 if (!therapistId) return null;
 try {
  const { data } = await client.get<BackendTherapistPublic>(
...
export const authApi = {
 ...
 async loginWithApple(
  ...
  void fullName;
  throw new Error(
   "Apple Sign-In isn't available yet — backend endpoint pending."
  );
 },
 ...
 async refresh(): Promise<AuthTokens | null> {
  ...
  const { data } = await client.post<{
   accessToken: string;
   refreshToken?: string;
  }>("/auth/refresh", { refresh: current.refreshToken });
...
export const therapistApi = {
 ...
 async getReviews(): Promise<TherapistReview[]> {
  ...
  const { data } = await client.get<BackendReview[]>(
...
export const blogApi = {
 ...
 async addComment(postId: string, comment: string): Promise<BlogReview> {
  const { data } = await client.post<BackendBlogReview>(
   `/blog/${postId}/review`,
   { comment }
  );
...
export const payoutApi = {
 ...
 async request(amount: number): Promise<Payout> {
  const { data } = await client.post<BackendPayout>(
   "/therapist/payout/request",
   { amount }
  );
  return mapPayout(data);
 },
...
export const availabilityApi = {
 ...
 async getOverrides(): Promise<ScheduleOverride[]> {
  const { data } = await client.get<BackendScheduleOverride[]>(
   "/therapist/slots/overrides"
  );
 ...
 */
function pickEchoedOtp(
 data: BackendSendOtp | null | undefined
): string | undefined {
...
export const sessionApi = {
 ...
 async addDocument(
  ...
  if (!data?.id)
   throw new Error("Upload succeeded but the server returned no document id.");
  return {
   id: data.id,
...
export type TreatmentSubmitPayload = Omit<
 Treatment,
...
function buildAssessmentInput(
 payload: TreatmentSubmitPayload
): ClinicalAssessmentInput {
 const notes = [
  payload.clinicalNotes,
  payload.precautions && `Precautions: ${payload.precautions}`,
 ]
  .filter(Boolean)
  .join("\n\n");
...
export const treatmentApi = {
 ...
 async getAssessments(planId: string): Promise<ClinicalAssessmentRecord[]> {
  const { data } = await client.get<BackendAssessment[]>(
   `/treatment-plan/${planId}/assessment`
  );
 ...
 async submitImprovementRecord(
  ...
  await client.post(
   ...
   {
    ...(record.painScoreBefore != null
     ? { painScoreBefore: clamp(record.painScoreBefore) }
     : {}),
    painScoreAfter: clamp(record.painScoreAfter),
    improvementNotes: record.improvementNotes,
    ...(record.exercisesGiven?.length
     ? { exercisesGiven: record.exercisesGiven }
     : {}),
   },
   idempotencyKey
    ? { headers: { "Idempotency-Key": idempotencyKey } }
...
export const notificationApi = {
 async list(): Promise<AppNotification[]> {
  const { data } = await client.get<BackendNotificationPage>(
   "/notifications/",
   {
    params: { limit: 50 },
   }
 ...
 async unreadCount(): Promise<number> {
  const { data } = await client.get<{ unreadCount?: number }>(
...
function normalizeUpload(data: BackendUpload | string | null | undefined): {
 ...
 return {
  url: absoluteFileUrl(raw),
  id: data?.id ?? data?.filename ?? data?.key ?? raw,
 };
}
